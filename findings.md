...
- 现有 `groupModeration` 已有后台开关、启用群、全局管理员和每群管理员配置，也会扫描文字、图片、合并转发和广告模板。
- 现有命中处理仅发送普通群内文字，并私聊转发证据；需要在群内提醒发送路径中构造 `at` 消息段。
- 为避免重复配置，将复用 `globalAdmins` 和 `groupAdmins` 作为通知名单；新增独立开关控制是否在群内艾特，默认开启但整个扫描仍默认关闭。
- 实现会排除广告发送者与机器人自身、去重后再发 `at` 段，因此不会自艾特或重复艾特同一人。
- 生产已最小化同步 5 个广告扫描文件；现有生产配置保留 `enabled: false`，新字段已补为 `mentionConfiguredAdminsInGroup: true`，等待后台配置启用群和名单。
- 用户确认的边界：群内艾特和私聊证据均只发送到后台填写的 QQ，绝不因为 QQ 群原生管理员/群主身份自动通知。
- 后台已上线明确为“全局通知 QQ”“每群通知 QQ”“群内艾特通知名单”和“私聊证据给通知名单”。
