...
const EXPLICIT_IMAGE_GENERATION_PATTERN = /(?:画图|画画|生图|出图|生成图片|生成照片|生成一?张(?:图|图片|照片|照)|画一?张|绘制一?张|做一?张(?:图|图片|照片|照)|从零(?:画|生成|绘制|做)|(?:帮我|给我|请|麻烦).{0,12}(?:画|绘制|生成|做).{0,80}(?:图|画|插画|图片|照片|角色)|(?:画|绘制|生成|做|捏)(?:一|1)?(?:个|张|幅|只|位)?.{1,180}(?:的)?(?:画|画面|图|图片|照片|插画|立绘)(?:$|[，,。.!！?？])|提示词(?:为|是|：|:))/i
